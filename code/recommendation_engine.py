...
FAISS IVF+PQ over CLAP 512-dim embeddings, with BPM/key re-rank.
Falls back to feature-vector cosine similarity when FAISS is unavailable.

All imports are flat — models.py, schemas.py are siblings in backend/.
Default paths point to D:/music_data/ with V3 file names.
model_name casing: "Clap" (matches 06_load_db.py loader).
"""
import os
import json
import logging
...
from models import Track, AudioFeature, SongEmbedding, Interaction, User
...
# ── Paths — default to D:/music_data/ with V3 file names ─────────────────────
EMBED_DIR        = Path(os.getenv("EMBED_DIR",  "/music_data/embeddings"))
FAISS_INDEX_PATH = os.getenv("FAISS_INDEX_PATH", "/music_data/index/faiss_ivfpqV3.index")
FAISS_MAP_PATH   = os.getenv("FAISS_ID_MAP_PATH", "/music_data/index/track_id_map_V3.json")
...
def _load_embedding_db(fma_track_id: int, db: Session) -> Optional[np.ndarray]:
    ...
    row = (
        ...
        .filter(
            ...
            SongEmbedding.model_name == "Clap",       # ← casing matches loader
        )
        .first()
    )
    if row is None:
    ...
    def find_similar_tracks(
        ...
        if faiss_ready() and seed.fma_track_id:
            ...
            if seed_vec is not None:
                ...
                scored = [x for x in scored if x[1] >= min_score]  # ← filter
                return [
        ...
        return self._fallback_similar(seed, seed_feat, limit, db, min_score)
    # ------------------------------------------------------------------
